...
import type { SuratPermohonanPindah, SuratPermohonanPindahSortableKeys, FindAllSuratPermohonanPindahDto } from '../../types/suratPermohonanPindah.types';

interface SuratPermohonanPindahTableProps {
    userList: SuratPermohonanPindah[];
    isLoading: boolean;
    onSort: (sortKey: SuratPermohonanPindahSortableKeys) => void;
    queryParams: Partial<FindAllSuratPermohonanPindahDto>;
    onEdit: (surat: SuratPermohonanPindah) => void;
    onDelete: (id: number) => void;
}

const SuratPermohonanPindahTable: React.FC<SuratPermohonanPindahTableProps> = ({
    userList,
    isLoading,
    onSort,
    queryParams,
                ...
                <table className="min-w-full text-left text-sm">
                    <thead className="bg-gray-50">
                        <tr className="text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th className="px-6 py-3 text-center">Surat Permohonan Pindah</th>
                        ...
                        {isLoading
                            ...
                            : userList.length === 0
                                ...
                                : userList.map((surat, index) => {
                                    const { filePmhnPindah, ...otherFiles } = surat;
                                        ...
                                        <React.Fragment key={surat.id}>
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-4 text-center">
                                                    {renderFileCell(surat.filePmhnPindah)}
                                                </td>
                                            ...
                                            <div className="bg-gray-50/70 px-4 py-3 border-b border-gray-200">
                                                <div className="flex items-center justify-between gap-4">
                                                    ...
                                                    <div className="flex items-center gap-1 flex-shrink-0">
                                                        <button onClick={() => onEdit(surat)} title="Edit Data" aria-label="Edit Data SuratPermohonanPindah" className="w-10 h-10 flex items-center justify-center rounded-lg bg-amber-100 text-amber-600 hover:bg-amber-200 transition-colors cursor-pointer">
                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                            </svg>
                                                        </button>
                                                        <button onClick={() => onDelete(surat.id)} title="Hapus Data" aria-label="Hapus Data SuratPermohonanPindah" className="w-10 h-10 flex items-center justify-center rounded-lg bg-red-100 text-red-600 hover:bg-red-200 transition-colors cursor-pointer">
                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path> </svg>
                                                        </button>
                                                    </div>
                                                </div>
...
export default SuratPermohonanPindahTable;
